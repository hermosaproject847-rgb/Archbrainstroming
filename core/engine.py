    ...
    for s in plan.stairs:
        ...
        if not s.show_dn and g.get("winder_polys"):
            ...
            if fl0["axis"] == "x":
                ...
                # the sheet's line RISES toward the outer wall: it springs
                # from the inner corner (band meets column, lower edge) up
                # to the outer side at 45 deg
                if fl0["dir"] > 0:
                    ti, cb = (cx0, cy0), (cx1, cyD)
                    ext = [(cx0, cy0), (cx0, cyT), (cx1, cyT), (cx1, cyD)]
                    wed = [(cx0, cy0), (cx1, cyD), (cx1, cy0)]
                else:
                    ti, cb = (cx1, cy0), (cx0, cyD)
                    ext = [(cx1, cy0), (cx1, cyT), (cx0, cyT), (cx0, cyD)]
                    wed = [(cx1, cy0), (cx0, cyD), (cx0, cy0)]
            else:
                cx0, cx1, cy0, cy1 = fx, fx + fw, ly, ly + lh
                cxT = _face_beyond(cx1, False, cy0, cy1)
                cxD = min(cx1, cx0 + _colw)
                if fl0["dir"] > 0:
                    ti, cb = (cx0, cy0), (cxD, cy1)
                    ext = [(cx0, cy0), (cxT, cy0), (cxT, cy1), (cxD, cy1)]
                    wed = [(cx0, cy0), (cxD, cy1), (cx0, cy1)]
                else:
                    ti, cb = (cx0, cy1), (cxD, cy0)
                    ext = [(cx0, cy1), (cxT, cy1), (cxT, cy0), (cxD, cy0)]
                    wed = [(cx0, cy1), (cxD, cy0), (cx0, cy0)]
...
def draw_furniture(plan: Plan, dl: DrawList) -> None:
    ...
    for f in plan.furniture:
        ...
        pw, ph = furn.printed_wh(plan, f)
        dl.text(cx, cy - 0.28,
                f"{_fmt_ft(pw)} x {_fmt_ft(ph)}", h=0.26, layer="FURNTAG")
